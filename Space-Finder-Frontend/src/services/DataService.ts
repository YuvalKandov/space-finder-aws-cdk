...
import { DataStack, ApiStack } from '../../../Space-Finder-Backend/outputs.json';
import { type SpaceEntry } from "../components/model/model";

const spacesUrl = ApiStack.ApiSpacesEndpointB4AFECE1 + 'spaces'
...
export class DataService {
    ...
    public async getSpaces(): Promise<SpaceEntry[]> {
        const res = await fetch(spacesUrl, {
          method: 'GET',
          headers: {
        ...
        if (!res.ok) {
          console.error('getSpaces failed:', json);
          return [];
        }
      
        return Array.isArray(json) ? json : (json.spaces ?? []);
      }
